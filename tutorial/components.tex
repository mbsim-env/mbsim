...
\begin{enumerate}
\item $\text{\texttt{DSS::plot}}\left(t,\vq,\vu\right)$
\item $\vq\leftarrow\vq+\text{\texttt{DS::deltaq}}\left(t,\vq,\vu\right)$
\item $t\leftarrow t+\Delta t$
\item $\text{\texttt{DSS::update}}\left(t,\vq,\vu\right)$
    \begin{enumerate}
    \item \texttt{DSS::updateStateDependentVariables}\\
      update variables depending on the generalised state of the system and the structure of one independent group and several trees
    \item \texttt{DSS::updateg}\\
      update of the relative position kinematics independent of the object structure\\
      \texttt{Link} $\rightarrow$ \texttt{Joint}, \texttt{Contact} $\rightarrow$ \texttt{ContactKinematics}\\
      several contacts points are possible from the kinematical point of view, whereby the maximum number is calculated in \texttt{ContactKinematics}\\
      convention: in the contact cartesian frame matrix the first column is the outpointing normal, the second column sign is different for the two contacting bodies 
    \end{enumerate}
\item $\text{\texttt{DSS::solveImpacts}}\left(t,\vq,\vu\right)$
\item $\vu\leftarrow\vu+\text{\texttt{DSS::deltau}}$
\item $\vx\leftarrow\vx+\text{\texttt{DSS::deltax}}$
\item \texttt{DSS::projectGeneralizedPositions}
\end{enumerate}
