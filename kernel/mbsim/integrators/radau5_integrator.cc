...
namespace MBSim {
  ...
  // This code is taken from radau5.f
  double RADAU5Integrator::delta(int i, double z) const {
    ...
    try { // catch exception -> C code must catch all exceptions
        ...
        if(self->shift) {
          ...
          while(dt>self->dtRoot) {
            ...
            for(int i=1; i<=self->system->getzSize(); i++)
              self->system->getState()(i-1) = CONTR5(&i,&tCheck,cont,lrc);
            self->system->resetUpToDate();
          ...
          if(curTimeAndState != tRoot) {
            ...
            for(int i=1; i<=self->system->getzSize(); i++)
              self->system->getState()(i-1) = CONTR5(&i,&tRoot,cont,lrc);
          }
          self->system->resetUpToDate();
      ...
      while(tRoot >= self->tPlot) {
        if(curTimeAndState != self->tPlot) {
          ...
          for(int i=1; i<=self->system->getzSize(); i++)
            self->system->getState()(i-1) = CONTR5(&i,&self->tPlot,cont,lrc);
        }
        self->system->resetUpToDate();
        if(self->formalism) {
          for(int i=self->system->getzSize()+1; i<=self->system->getzSize()+self->system->getlaSize(); i++)
            self->system->getla(false)(i-(self->system->getzSize()+1)) = CONTR5(&i,&self->tPlot,cont,lrc);
          self->system->setUpdatela(false);
        }
        self->system->plot();
