  ...
  void DOP853Integrator::integrate() {
    ...
    if(z0.size()) {
     if(z0.size() != system->getzSize()+system->getisSize())
        throwError("(DOP853Integrator::integrate): size of z0 does not match, must be " + to_string(system->getzSize()+system->getisSize()));
    ...
    if(aTol.size() == 1)
      ...
      iTol = 1;
      if(aTol.size() != zSize)
        throwError("(DOP853Integrator::integrate): size of aTol does not match, must be " + to_string(zSize));
    }
    if(rTol.size() != aTol.size())
      throwError("(DOP853Integrator::integrate): size of rTol does not match aTol, must be " + to_string(aTol.size()));
    ...
    while(t<tEnd-epsroot) {
      ...
      if(idid < 0)
        throw runtime_error("DOP853 failed with idid = "+to_string(idid));
