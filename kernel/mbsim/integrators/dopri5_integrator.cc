...
namespace MBSim {
    ...
    try { // catch exception -> C code must catch all exceptions
      ...
      if(self->system->getsvSize()) {
        ...
        if(self->shift) {
          ...
          while(dt>self->dtRoot) {
            ...
            for(int i=1; i<=*n; i++)
              self->system->getState()(i-1) = CONTD5(&i,&tCheck,con,icomp,nd);
            self->system->resetUpToDate();
          ...
          if(curTimeAndState != tRoot) {
            ...
            for(int i=1; i<=*n; i++)
              self->system->getState()(i-1) = CONTD5(&i,&tRoot,con,icomp,nd);
          }
          self->system->resetUpToDate();
      ...
      while(tRoot >= self->tPlot) {
        if(curTimeAndState != self->tPlot) {
          ...
          for(int i=1; i<=*n; i++)
            self->system->getState()(i-1) = CONTD5(&i,&self->tPlot,con,icomp,nd);
        }
        self->system->resetUpToDate();
        self->system->plot();
  ...
  void DOPRI5Integrator::integrate() {
    ...
    if(not zSize)
      throwError("(DOPRI5Integrator::integrate): dimension of the system must be at least 1");
    ...
    if(z0.size()) {
      if(z0.size() != system->getzSize()+system->getisSize())
        throwError("(DOPRI5Integrator::integrate): size of z0 does not match, must be " + to_string(system->getzSize()+system->getisSize()));
