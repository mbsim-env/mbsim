...
namespace MBSimIntegrator {
  ...
  void DOPRI5Integrator::integrate() {
    ...
    if(aTol.size() == 1)
      ...
      iTol = 1;
      if(aTol.size() != zSize)
        throw MBSimError(string("(DOPRI5Integrator::integrate): size of aTol does not match, must be ") + toStr(zSize));
    }
    if(rTol.size() != aTol.size())
      throw MBSimError(string("(DOPRI5Integrator::integrate): size of rTol does not match aTol, must be ") + toStr(aTol.size()));
