...
//    self->getSystem()->setState(z); Not needed as the integrator uses the state of the system
  ...
  void LSODARIntegrator::preIntegrate() {
    ...
    if(z0.size()) {
      if(z0.size() != zSize)
        throwError("(LSODARIntegrator::integrate): size of z0 does not match " + to_string(zSize));
      system->setState(z0);
    }
    else
      system->evalz0();
//    system->setState(z); Not needed as the integrator uses the state of the system
    ...
    if(rTol.size() == 1) {
      if(aTol.size() == 1)
        ...
        iTol = 2;
        if(aTol.size() != zSize)
          throwError("(LSODARIntegrator::integrate): size of aTol does not match, must be " + to_string(zSize));
      }
    }
    else {
      if(aTol.size() == 1)
      ...
      if(rTol.size() != zSize)
        throwError("(LSODARIntegrator::integrate): size of rTol does not match, must be " + to_string(zSize));
    }
...
//          system->setState(z); Not needed as the integrator uses the state of the system
      ...
      else if(istate<0) throwError("Integrator LSODAR failed with istate = "+to_string(istate));
    }
  }
