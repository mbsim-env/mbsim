...
namespace MBSim {
  ...
  void ThetaTimeSteppingIntegrator::preIntegrate() {
    ...
    if(z0.size()) {
      if(z0.size() != system->getzSize())
        throwError("(ThetaTimeSteppingIntegrator::integrate): size of z0 does not match, must be " + to_string(system->getzSize()));
      system->setState(z0);
    }
    else
      system->evalz0();
  ...
  void ThetaTimeSteppingIntegrator::subIntegrate(double tStop) {
    while(system->getTime()<tStop) { // time loop
      ...
      system->getGs(false).reassign(system->getG(false));
