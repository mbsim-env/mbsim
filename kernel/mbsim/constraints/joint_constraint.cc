...
namespace MBSim {
  ...
  Vec JointConstraint::Residuum::operator()(const Vec &x) {
    ...
    // T_RRes -> transforms from (R)refFrame-system [output of AIK2Cardan(getOriantation.T()*getOrientation())] to (Res)residuals
    Mat3xV dR = momentDir;
    ...
    if(dR.cols())
      res.set(Range<Var,Var>(dT.cols(),dT.cols()+dR.cols()-1), dR.T()*AIK2Cardan(frame[refFrame]->getOrientation().T()*frame[refFrame == firstFrame ? secondFrame : firstFrame]->getOrientation()));
