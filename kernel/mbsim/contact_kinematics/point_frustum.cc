...
namespace MBSim {
  ...
  void ContactKinematicsPointFrustum::updateg(double t, double &g, std::vector<ContourFrame*> &cFrame, int index) {
    ...
    Vec3 Wd = point->getFrame()->IrOP() - frustum->getFrame()->IrOP(); // difference vector of Point and Frustum basis point in inertial FR
    Vec3 Wa = frustum->getFrame()->AIK().col(1); // (height) axis in inertial FR
