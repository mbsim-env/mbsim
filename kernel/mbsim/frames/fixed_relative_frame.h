...
namespace MBSim {
  ...
  class FixedRelativeFrame : public Frame {
    ...
    public:
      FixedRelativeFrame(const std::string &name = "dummy", const fmatvec::Vec3 &r=fmatvec::Vec3(), const fmatvec::SqrMat3 &A=fmatvec::SqrMat3(fmatvec::EYE), Frame *refFrame=nullptr) : Frame(name), R(refFrame), RrRP(r), ARP(A) {
      }
