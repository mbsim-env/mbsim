...
namespace MBSimFlexibleBody {
  ...
  SuperElementLinearExternal::SuperElementLinearExternal() :
      MBSim::DiscretizationInterface<Ref>(), M(0), K(0), alpha(0.), beta(0.) {
  }
  ...
  void SuperElementLinearExternal::setM(const SymMat &M_) {
    if (M_.size() == getqSize() || getqSize() == 0)
      M.resize() = M_;
    else
      throw MBSimError("Massmatrix and stiffnessmatrix have unequal sizes!!!");
  }

  void SuperElementLinearExternal::setK(const SqrMat &K_) {
    if (K_.size() == getuSize() || getuSize() == 0)
      K.resize() = K_;
    else
      throw MBSimError("Massmatrix and stiffnessmatrix have unequal sizes!!!");
  }
  
  void SuperElementLinearExternal::setRayleighDamping(double alpha_, double beta_) {
    alpha = alpha_;
    beta = beta_;
  }

  void SuperElementLinearExternal::computeh(const Vec & qElement, const Vec & uElement) {
    h = -K * qElement - D * uElement;
  }

  Vec3 SuperElementLinearExternal::computePosition(const Vec&q, const ContourPointData& cp) {
    return static_cast<RowVec3>(q * computeJacobianOfMotion(q, cp)).T()  + KrP[cp.getNodeNumber()];
  }
