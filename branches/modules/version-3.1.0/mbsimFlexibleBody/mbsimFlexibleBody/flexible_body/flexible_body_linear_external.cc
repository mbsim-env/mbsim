...
namespace MBSimFlexibleBody {
  ...
  FlexibleBodyLinearExternal::FlexibleBodyLinearExternal(const string &name) :
          FlexibleBody(name), nContours(0), WrON00(Vec(3)) {
  ...
  void FlexibleBodyLinearExternal::init(InitStage stage) {

    static_cast<SuperElementLinearExternal*>(discretization[0])->init(stage);

    if(stage == resize) {
      qElement[0].resize(qSize);
      uElement[0].resize(uSize[0]);

      LLM[0] = facLL(static_cast<SuperElementLinearExternal*>(discretization[0])->getM());
    }

    FlexibleBody::init(stage);
  }

  void FlexibleBodyLinearExternal::facLLM(int i) {
    LLM[i] = facLL(static_cast<SuperElementLinearExternal*>(discretization[0])->getM());
  }

  void FlexibleBodyLinearExternal::plot(double t, double dt) {
    if(getPlotFeature(plotRecursive) == enabled) {
      ...
      if(getPlotFeature(openMBV) == enabled && openMBVBody) {
        ...
        vector<double> data;
        data.push_back(t);
        //TODO: everything only possible for spine extrusion
        for(size_t i=0; i < CR.size(); i++) {
          Vec3 pos = rConst[i] + CR[i] * q;
  ...
  void FlexibleBodyLinearExternal::setProportionalDamping(const double &a, const double &b) {
    static_cast<SuperElementLinearExternal*>(discretization[0])->setRayleighDamping(a, b);
  }
  ...
  void FlexibleBodyLinearExternal::updateStateDependentVariables(double t) {
    if (nrm2(qElement[0] - q) > 0) {
      qElement[0] >> q;
    }
    if (nrm2(uElement[0] - u) > 0)
      uElement[0] >> u;
...
#ifdef HAVE_OPENMBVCPPINTERFACE
  void FlexibleBodyLinearExternal::setOpenMBVBody(OpenMBV::Body * openMBVBody_) {
    openMBVBody = openMBVBody_;
  }

  void FlexibleBodyLinearExternal::addVisualisationPoint(const Mat3V & CR_, const Vec3 & rConst_) {
    CR.push_back(CR_);
    rConst.push_back(rConst_);
  }
#endif
  //----------------------------------------------------------------------------------------
