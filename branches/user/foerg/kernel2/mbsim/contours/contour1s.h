...
namespace MBSim {
  ...
  class Contour1s : public ContourContinuum<double> {
    public:
       ...
       */
      virtual fmatvec::FVec computeTangent(ContourPointData &cp) { updateKinematicsForFrame(cp,firstTangent); return cp.getFrameOfReference().getOrientation().col(1); }

      /**
       * \return binormal in world frame
       * \param Lagrangian position
       */
      virtual fmatvec::FVec computeBinormal(ContourPointData &cp) { updateKinematicsForFrame(cp,secondTangent); return cp.getFrameOfReference().getOrientation().col(2); }
      /***************************************************/
