...
Joint::Joint(const QString &str, QTreeWidgetItem *parentItem, int ind) : Link(str, parentItem, ind) {
  ...
  force = new ExtWidget("Force",new GeneralizedForceChoiceWidget(forceArrow),true);
  properties->addToTab("Kinetics", force);

  moment = new ExtWidget("Moment",new GeneralizedForceChoiceWidget(momentArrow),true);
