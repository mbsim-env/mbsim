...
namespace MBSim {
  ...
  // template argument is undefined! Hence we define first the one with the less general form (but being the fastest).
  // If it comes to an dimension error during the initialization of this Function we just try it with the next one.
  MBSIM_OBJECTFACTORY_REGISTERXMLNAME_AND_INSTANTIATE(BinaryNestedFunction<Vec3   (double(VecV  ))>, MBSIM%"BinaryNestedFunction")
  MBSIM_OBJECTFACTORY_REGISTERXMLNAME_AND_INSTANTIATE(BinaryNestedFunction<Vec3   (VecV  (VecV  ))>, MBSIM%"BinaryNestedFunction")
  // The following functions are created using ...create<Function<Vec3(double)> >(...). Hence the "second"
  // template argument is undefined! Hence we define first the one with the less general form (but being the fastest).
  // If it comes to an dimension error during the initialization of this Function we just try it with the next one.
  MBSIM_OBJECTFACTORY_REGISTERXMLNAME_AND_INSTANTIATE(BinaryNestedFunction<Vec3   (double(double))>, MBSIM%"BinaryNestedFunction")
  MBSIM_OBJECTFACTORY_REGISTERXMLNAME_AND_INSTANTIATE(BinaryNestedFunction<Vec3   (VecV  (double))>, MBSIM%"BinaryNestedFunction")
  // The following functions are created using ...create<Function<RotMat3(VecV)> >(...). Hence the "second"
  // template argument is undefined! Hence we define first the one with the less general form (but being the fastest).
  // If it comes to an dimension error during the initialization of this Function we just try it with the next one.
  MBSIM_OBJECTFACTORY_REGISTERXMLNAME_AND_INSTANTIATE(BinaryNestedFunction<RotMat3(double(VecV  ))>, MBSIM%"BinaryNestedFunction")
  MBSIM_OBJECTFACTORY_REGISTERXMLNAME_AND_INSTANTIATE(BinaryNestedFunction<RotMat3(VecV  (VecV  ))>, MBSIM%"BinaryNestedFunction")
  // The following functions are created using ...create<Function<RotMat3(double)> >(...). Hence the "second"
  // template argument is undefined! Hence we define first the one with the less general form (but being the fastest).
  // If it comes to an dimension error during the initialization of this Function we just try it with the next one.
  MBSIM_OBJECTFACTORY_REGISTERXMLNAME_AND_INSTANTIATE(BinaryNestedFunction<RotMat3(double(double))>, MBSIM%"BinaryNestedFunction")
  MBSIM_OBJECTFACTORY_REGISTERXMLNAME_AND_INSTANTIATE(BinaryNestedFunction<RotMat3(VecV  (double))>, MBSIM%"BinaryNestedFunction")
  // The following functions are created using ...create<Function<double(double)> >(...). Hence the "second"
  // template argument is undefined! Hence we define first the one with the less general form (but being the fastest).
  // If it comes to an dimension error during the initialization of this Function we just try it with the next one.
  MBSIM_OBJECTFACTORY_REGISTERXMLNAME_AND_INSTANTIATE(BinaryNestedFunction<double(double(double))>, MBSIM%"BinaryNestedFunction")
  MBSIM_OBJECTFACTORY_REGISTERXMLNAME_AND_INSTANTIATE(BinaryNestedFunction<double(VecV  (double))>, MBSIM%"BinaryNestedFunction")
  // The following functions are created using ...create<Function<VecV(double)> >(...). Hence the "second"
  // template argument is undefined! Hence we define first the one with the less general form (but being the fastest).
  // If it comes to an dimension error during the initialization of this Function we just try it with the next one.
  MBSIM_OBJECTFACTORY_REGISTERXMLNAME_AND_INSTANTIATE(BinaryNestedFunction<VecV(double(double))>, MBSIM%"BinaryNestedFunction")
  MBSIM_OBJECTFACTORY_REGISTERXMLNAME_AND_INSTANTIATE(BinaryNestedFunction<VecV(VecV(double))>, MBSIM%"BinaryNestedFunction")
}
