...
MainWindow::MainWindow(QStringList &arg) : inlineOpenMBVMW(0) {
 ...
 // toolBar->addAction(actionH5plotserie);
  QAction *actionUpdate3DView = toolBar->addAction(Utils::QIconCached(QString::fromStdString((MBXMLUtils::getInstallPath()/"share"/"mbsimgui"/"icons"/"h5plotserie.svg").string())),"Update 3D view");
...
//  toolBar->addAction(actionUpdate3DView);
  ...
  elementList->setColumnWidth(0,150);
  elementList->setColumnWidth(1,150);
...
//  QModelIndex index = elementList->currentIndex();
...
void MainWindow::parameterListClicked() {
  if(QApplication::mouseButtons()==Qt::RightButton) {
    QModelIndex index = parameterList->currentIndex();
    if(index.column()==0) {
...
void MainWindow::propertyListClicked() {
  if(QApplication::mouseButtons()==Qt::RightButton) {
    QModelIndex index = propertyList->currentIndex();
