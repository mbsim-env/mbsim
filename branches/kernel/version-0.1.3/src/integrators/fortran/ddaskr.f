C DASKR from netlib.org
C in MBSim imported source files: ddaskr.f daux.f (same as needed forddaspk.f)
C adaptions:
C		DATV   (ddaskr.f) renamed in DATV2   (conflict with opkda1.f)
C               DROOTS (ddaskr.f) renamed in DROOT2  (conflict with opkda1.f)
C               DRCHEK (ddaskr.f) renamed in DRCHEK2 (conflict with opkda1.f)
C		DHELS  (ddaskr.f) commented out      (opkda1.f routine is used instead)
C		DHEQR  (ddaskr.f) commented out      (opkda1.f routine is used instead)
...
C   040518  Changed adjustment to X2 in Subr. DROOT2.
...
C          interface between Subroutine DDASKR and Subroutine DROOT2.
C   DROOT2 finds the leftmost root of a set of functions.
...
C
     ...
     *   LMXNJ=33, LMXNH=34, LLSOFF=35, LNRTE=36, LIRFND=37, LIND1=45, 
     *   LIND2=46, LIND3=47, LICNS=51)
C
...
726   MSG = 'DASKR--  ILLEGAL IWORK VALUE FOR INFO(10) .NE. 0'
      ...
      GO TO 750
727   MSG = 'DASKR--  Y(I) AND IWORK(50+I) (I=I1) INCONSISTENT'
...
C***REFER TO DDASKR
C***ROUTINES CALLED  DDATRP, DROOT2, DCOPY, RT
C***REVISION HISTORY  (YYMMDD)
C   020815  DATE WRITTEN   
...
C input flag JOB.  It calls subroutine DROOT2 to locate the root
...
C Set T1 to TN or TOUT, whichever comes first, and get R at T1. --------
 ...
 330  CALL DDATRP (TN, T1, Y, YP, NEQ, KOLD, PHI, PSI)
      ...
      IWORK(LNRTE) = IWORK(LNRTE) + 1
C Call DROOT2 to search for root in interval from T0 to T1. ------------
      JFLAG = 0
 350  CONTINUE
      CALL DROOT2 (NRT, HMINR, JFLAG, RWORK(LT0),T1, R0,R1,RX, X, JROOT)
...
C---------------------- END OF SUBROUTINE DRCHEK2 -----------------------
      END
      SUBROUTINE DROOT2 (NRT, HMIN, JFLAG, X0, X1, R0, R1, RX, X, JROOT)
C
C***BEGIN PROLOGUE  DROOT2
C***REFER TO DRCHEK2
...
C are communicated through the call sequence of DROOT2.
...
C          JFLAG = 1 means DROOT2 needs a value of R(x).  Set RX = R(X)
C                    and call DROOT2 again.
...
C----------------------- END OF SUBROUTINE DYYPNW ----------------------
     ...
     *  NTYPE,NLS,INFO2,INFO16,RTOL,ATOL,NRIND1,NRIND2,NRIND3)
C
...
C***REVISION DATE  080727   (YYMMDD) H dependened error test added (R. Huber)
C
C
C-----------------------------------------------------------------------
C***DESCRIPTION
...
C
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      DIMENSION Y(*),YPRIME(*),WT(*),VT(*), RTOL(*), ATOL(*)
...
C ADDED LINES BELOW:  HR 24.07.2008
      IF (INFO16 .EQ. 2) THEN
        CALL DCALCVT(NEQ,H,INFO2,RTOL,ATOL,Y,VT,NRIND1, NRIND2, NRIND3,
     *         RPAR, IPAR)
        CALL DINVWT(NEQ,VT,IER)
      IF (IER .NE. 0) THEN
        IDID = -2
        GO TO 675
      ENDIF
...
C------END OF SUBROUTINE DDAWTS-----------------------------------------
      END
      SUBROUTINE DCALCVT(NEQ,HSCALE,IWT,RTOL,ATOL,Y,WT,NIND1,NIND2,
     *   NIND3,RPAR,IPAR)

C***BEGIN PROLOGUE  DCALCVT
C***REFER TO  DDASPK
C***ROUTINES CALLED  (NONE)
C***DATE WRITTEN   080722   (YYMMDD)    
C***END PROLOGUE  DCALCVT
C-----------------------------------------------------------------------
C     Description:
C
C     This subroutine sets the error weight vector, WT, for error test
C     with stepsize scaled variables.
C     It is usefull to sclae variables with the stepsize accrding to
C     their index:
C     index 1 varibales remain unscaled
C     index 2 varibales are scaled with stepsize HSCALE      
C     index 3 varibales are scaled with HSCALE**2
C     This means for the weight vector WT:
C       WT(I)=RTOL(I)*ABS(Y(I))+ATOL(I)                  Y(I): index 1
C       WT(I)=RTOL(I)*ABS(Y(I))+ATOL(I)/HSCAlE           Y(I): index 2
C       WT(I)=RTOL(I)*ABS(Y(I))+ATOL(I)/HSCAlE/HSCAlE    Y(I): index 3
C     See to Hairer, Lubich, Roche (1989): The numerical solution
C     of differential algebraic systems by Runge-Kutta methods.
C     Lecture Notes in Math. 1409, Springer for justification.
C
C     NEQ    : number of equations (dimension of Y)
C     HSCALE : stepsize used as sclae factor
C     IWT    : 0 scalar tolerances or 1 for vector tol. (=info(2))
C     RTOL   : relative tolerances
C     ATOL   : absolute tolerances
C     Y      : solution
C     WT     : weight vector (OUTPUT)
C     NIND1  : number of index 1 variables
C     NIND2  : number of index 2 variables
C     NIND3  : number of index 3 variables
C
C  !!! ATTENTION !!      
C       THE RES FUNCTION-SUBROUTINE SHOULD BE WRITTEN SUCH THAT
...
20         CONTINUE
      DO 40 I=1,NIND2
         II= NIND1+I
         IF (IWT .EQ.0) GO TO 30
           RTOLI=RTOL(II)
           ATOLI=ATOL(II)
30         WT(II)=RTOLI*ABS(Y(II))+ATOLI/HSCALE  
40         CONTINUE
      DO 60 I=1,NIND3
         II= NIND1+NIND2+I
         IF (IWT .EQ.0) GO TO 50
           RTOLI=RTOL(II)
           ATOLI=ATOL(II)
50         WT(II)=RTOLI*ABS(Y(II))+ATOLI/HSCALE/HSCALE
60         CONTINUE
      RETURN
C
C------END OF SUBROUTINE DCALCVT-----------------------------------------
      END
...
C      END
C      
C  PROTOCOL OF MODIFICATIONS MADE BY ROBERT HUBER (HR)
C  =====================================================  

CSUBROUTINE DDASKR
C
CLine 466: Documentation INFO(10)
C40 replaced with 50
CC                  IWORK(50+I) = +1 if Y(I) must be .GE. 0,
CC                  IWORK(50+I) = +2 if Y(I) must be .GT. 0,
CC                  IWORK(50+I) = -1 if Y(I) must be .LE. 0, while
CC                  IWORK(50+I) = -2 if Y(I) must be .LT. 0, while
CC                  IWORK(50+I) =  0 if Y(I) is not constrained.
C
C
CLine 501: Documentation INFO(11)
C40 replaced with 50
CC                  IWORK(LID+I) = -1 if Y(I) is an algebraic variable,
CC                  where LID = 50 if INFO(10) = 0 or 2 and LID = 50+NEQ
CC                  if INFO(10) = 1 or 3.
C
CLine 588: Documentation INFO(16)
Cadded optin INFO(16)=2 
CC HR 24.07.2008
CC                set INFO(16) = 2 to scale components with stepsize H
CC		                for stepsize control
CC		 index 2 variables are scaled with H
CC		index 3 variables are scaled wuth H^2
CC	        The index 1,2,3 variables should appear in this order
CC		The dimensions must be specified in IWORK(45), IWORK(46)
CC		and IWORK(47)
CC	        IWORK(45): dimension of index 1 variables (=NEQ for ODE)
CC		IWORK(46): dimension of index 2 variables
CC		IWORK(47): dimension of index 3 variables
C
CLine 610: Documentation INFO(16)
C40 replaced with 50
CC                       where LID = 50 if INFO(10) = 0 or 2 and 
CC                       LID = 50 + NEQ if INFO(10) = 1 or 3.
C
CLine 763 and 768: Documentation LIW
C40 replaced with 50
CC             base = 50 + NEQ.
CC             IF INFO(10) = 1 or 3, add NEQ to the base value.
CC             If INFO(11) = 1 or INFO(16) =1, add NEQ to the base value.
CC
CC             If INFO(12) = 1 (Krylov method), the base value is
CC             base = 50 + LENIWP.
C
CLine 1447: code
Cbasis of IWORK extended from 40 to 50 and LIND1, LIND2 and LIND3 added
CC     Set pointers into IWORK.
CC     HR 18/07/2008 extended basis of IWORK to 50 and added LIND1..3 
CC
C      PARAMETER (LML=1, LMU=2, LMTYPE=4, 
C     *   LIWM=1, LMXORD=3, LJCALC=5, LPHASE=6, LK=7, LKOLD=8,
C     *   LNS=9, LNSTL=10, LNST=11, LNRE=12, LNJE=13, LETF=14, LNCFN=15,
C     *   LNCFL=16, LNIW=17, LNRW=18, LNNI=19, LNLI=20, LNPS=21,
C     *   LNPD=22, LMITER=23, LMAXL=24, LKMP=25, LNRMAX=26, LLNWP=27,
C     *   LLNIWP=28, LLOCWP=29, LLCIWP=30, LKPRIN=31, LMXNIT=32,
C     *   LMXNJ=33, LMXNH=34, LLSOFF=35, LNRTE=36, LIRFND=37, LIND1=45, 
C     *   LIND2=46, LIND3=47, LICNS=51)
C
CLine 1489: code
Ccheck of INFO(16) modified (.EG.2 now possible!)
C     DO 15 I=12,15
C         ITEMP = I
C         IF (INFO(I) .NE. 0 .AND. INFO(I) .NE. 1) GO TO 701
C 15      CONTINUE
C      ITEMP = 16
C      IF (INFO(16).LT.0 .OR. INFO(16).GT.2) GO TO 701
C      ITEMP = 17
C      IF (INFO(17) .NE. 0 .AND. INFO(17) .NE. 1) GO TO 701
C
CLine 1505: code
CC     Check dimension of index 1,2,3 variables (HR 24.07.2008)
C      SUMLIND = IWORK(LIND1)+IWORK(LIND2)+IWORK(LIND3)
C      IF(SUMLIND .NE. NEQ) THEN
C        WRITE(6,*)' SUM IWORK(45,46,47) .NE. NEQ'
C        RETURN
C      ENDIF
C
CLine 1595: code
CC LINE BELOW: 24.07.2008 HR Changed from INFO(16).EQ. 1 to .NE. 0 
C      IF (INFO(11) .EQ. 1 .OR. INFO(16) .NE. 0) LENID = NEQ
C
CLine 1627 and 1639: code
C40 replaced with 50
C         LENIW = 50 + LENIC + LENID + NEQ
C...
C         LENIW = 50 + LENIC + LENID + LENIWP
C
C
CLine 1967: code
Ccomputation weight vector added
CC    compute VT(H) 
CC ADDED LINES BELOW:  HR 24.07.2008
C      IF (INFO(16) .EQ. 2) THEN
C        CALL DCALCVT(NEQ,H,INFO(2),RTOL,ATOL,Y,RWORK(LVT),IWORK(LIND1),
C     *        IWORK(LIND2), IWORK(LIND3), RPAR, IPAR)
C        CALL DINVWT(NEQ,RWORK(LVT),IER)
C      IF (IER .NE. 0) GO TO 713
C      ENDIF
C
C
CLine 2194: code
...
C
CLine 2209: code
Cadditonal arguments in call of subroutine DDSTP
C        CALL DDSTP(TN,Y,YPRIME,NEQ,
C     *      RES,JAC,PSOL,H,RWORK(LWT),RWORK(LVT),INFO(1),IDID,RPAR,IPAR,
C     *      RWORK(LPHI),RWORK(LSAVR),RWORK(LDELTA),RWORK(LE),
C     *      RWORK(LWM),IWORK(LIWM),
C     *      RWORK(LALPHA),RWORK(LBETA),RWORK(LGAMMA),
C     *      RWORK(LPSI),RWORK(LSIGMA),
C     *      RWORK(LCJ),RWORK(LCJOLD),RWORK(LHOLD),RWORK(LS),HMIN,
C     *      RWORK(LROUND), RWORK(LEPLI),RWORK(LSQRN),RWORK(LRSQRN),
C     *      RWORK(LEPCON), IWORK(LPHASE),IWORK(LJCALC),INFO(15),
C     *      IWORK(LK), IWORK(LKOLD),IWORK(LNS),NONNEG,INFO(12),
C     *      DNEDD,
C     *      INFO(2), INFO(16), RTOL, ATOL, IWORK(LIND1), 
C     *      IWORK(LIND2), IWORK(LIND3))
C      ELSE IF (INFO(12) .EQ. 1) THEN
C         CALL DDSTP(TN,Y,YPRIME,NEQ,
C     *      RES,JAC,PSOL,H,RWORK(LWT),RWORK(LVT),INFO(1),IDID,RPAR,IPAR,
C     *      RWORK(LPHI),RWORK(LSAVR),RWORK(LDELTA),RWORK(LE),
C     *      RWORK(LWM),IWORK(LIWM),
...
C     *      IWORK(LIND2), IWORK(LIND3))
C
CLine 2539: code for error handling
C727   MSG = 'DASPK--  Y(I) AND IWORK(50+I) (I=I1) INCONSISTENT'
C
C---------------------------------------------------------------------------
C
CSUBROUTINE DDSTP
C
CLine 3199: arguments in definition of DDSTP
Cadded arguments INFO2,INFO16,RTOL,ATOL,NRIND1,NRIND2 and NRIND3
...
C
CLine 3210: documentation of DDSTP
CC***REVISION DATE  080727   (YYMMDD) H dependened error test added (R. Huber)
CC
C
C
CLine 3273: code DDSTP
Cdeclaration fo RTOL and ATOL as arrays
C      DIMENSION Y(*),YPRIME(*),WT(*),VT(*), RTOL(*), ATOL(*)
C
CLine 3413: code DDSTP
Cadded calculation VT
CC    compute VT(H) 
CC ADDED LINES BELOW:  HR 24.07.2008
...
C
C---------------------------------------------------------------------------------
CLine 3895: SUBROUTINE DCALCVT added!
C
