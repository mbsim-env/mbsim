...
Property* TolerancePropertyFactory::createProperty(int i) {
  if(i==0) {
    ...
    input.push_back(PhysicalVariableProperty(new ScalarProperty("","1e-6",Units()),"",MBSIMINT%(type+"ToleranceScalar")));
    return new ExtPhysicalVarProperty(input);
  }
  if(i==1) {
    vector<PhysicalVariableProperty> input;
    input.push_back(PhysicalVariableProperty(new VecProperty(0,Units()),"",MBSIMINT%(type+"Tolerance")));
    return new ExtPhysicalVarProperty(input);
  }
}
...
Integrator::Integrator() : initialState(0,false) {
  ...
  vector<PhysicalVariableProperty> input;
  input.push_back(PhysicalVariableProperty(new ScalarProperty("","0",TimeUnits()),"s",MBSIMINT%"startTime"));
  startTime.setProperty(new ExtPhysicalVarProperty(input)); 

  input.clear();
  input.push_back(PhysicalVariableProperty(new ScalarProperty("","1",TimeUnits()),"s",MBSIMINT%"endTime"));
  endTime.setProperty(new ExtPhysicalVarProperty(input)); 

  input.clear();
  input.push_back(PhysicalVariableProperty(new ScalarProperty("","1e-2",TimeUnits()),"s",MBSIMINT%"plotStepSize"));
...
DOPRI5Integrator::DOPRI5Integrator() : maxSteps(0,false) {
  ...
  vector<PhysicalVariableProperty> input;
  input.push_back(PhysicalVariableProperty(new ScalarProperty("","0",TimeUnits()),"s",MBSIMINT%"initialStepSize"));
  initialStepSize.setProperty(new ExtPhysicalVarProperty(input)); 

  input.clear();
  input.push_back(PhysicalVariableProperty(new ScalarProperty("","0",TimeUnits()),"s",MBSIMINT%"maximalStepSize"));
  maximalStepSize.setProperty(new ExtPhysicalVarProperty(input)); 

  input.clear();
  input.push_back(PhysicalVariableProperty(new ScalarProperty("","0",Units()),"",MBSIMINT%"maximalNumberOfSteps"));
