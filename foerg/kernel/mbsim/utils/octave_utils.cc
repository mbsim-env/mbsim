  ...
  OctaveComplexMatrix* OctaveParser::readComplexMatrix() {
    ...
    for(int i=0; i<m; i++) {
      for(int j=0; j<n; j++) {
        is >> buf;
        int pos = buf.find(",");
        A(i,j) = complex<double>(atof(buf.substr(1,pos-1).c_str()),atof(buf.substr(pos+1,buf.length()-pos-2).c_str()));
      }
    }
