...
{
  cs *C, *A2, *AT;
  int *Cp, *Ci, *last, *ww, *len, *nv, *next, *P, *head, *elen, *degree, *w, *hhead, *ATp, *ATi, d, dk, dext, lemax = 0, e, elenk, eln, i, j, k, k1, k2, k3, jlast, ln, dense, nzmax, mindeg = 0, nvi, nvj, nvk, mark, wnvi, ok, cnz, nel = 0, p, p1, p2, p3, p4, pj, pk, pk1, pk2, pn, q, n, m;
  unsigned int h;
  /* --- Construct matrix C ----------------------------------------------- */
  if (!A || order < 0)
    return (NULL ); /* check inputs; quick return */
  ...
  Cp = C->p;
  cnz = Cp[n];
  if (!cs_sprealloc(C, cnz + cnz / 5 + 2 * n))
    return (cs_idone(P, C, ww, 0));
...
static
int cs_ereach(const cs *A, int k, const int *parent, int *s, int *w, double *x, int top) {
...
int *cs_counts(const cs *A, const int *parent, const int *post, int ata) {
  int i, j, k, p, n, m, ii, s, *ATp, *ATi, *maxfirst, *prevleaf, *ancestor, *head = NULL, *next = NULL, *colcount, *w, *first, *delta;
  cs *AT;
  if (!A || !parent || !post)
    return (NULL ); /* check inputs */
  m = A->m;
...
static int cs_bfs(const cs *A, int n, int *wi, int *wj, int *queue, const int *imatch, const int *jmatch, int mark) {
  ...
  if (!C)
    return (0); /* bfs of C=A' to find R0,R3,C3 */
  Ap = C->p;
  Ai = C->i;
...
static void cs_matched(int m, const int *wi, const int *jmatch, int *P, int *Q, int *cc, int *rr, int set, int mark) {
  ...
  for (i = 0; i < m; i++) {
    if (wi[i] != mark)
      continue; /* skip if i is not in R set */
    P[kr++] = i;
    Q[kc++] = jmatch[i];
  }
  cc[set + 1] = kc;
...
csd *cs_dmperm(const cs *A) {
  int m, n, i, j, k, p, cnz, nc, *jmatch, *imatch, *wi, *wj, *Pinv, *Cp, *Ci, *Ps, *Rs, nb1, nb2, *P, *Q, *cc, *rr, *R, *S, ok;
  cs *C;
  csd *D, *scc;
  /* --- Maximum matching ------------------------------------------------- */
  if (!A)
...
csn *cs_lu(const cs *A, const css *S, double tol) {
  ...
  for (k = 0; k < n; k++) /* compute L(:,k) and U(:,k) */
  {
    ...
    if ((lnz + n > L->nzmax && !cs_sprealloc(L, 2 * L->nzmax + n)) || (unz + n > U->nzmax && !cs_sprealloc(U, 2 * U->nzmax + n))) {
      return (cs_ndone(N, NULL, xi, x, 0));
    }
    Li = L->i;
    Lx = L->x;
...
{
  int i, j, k, n, m, p, n2 = 0, m2 = 0, *Ap, *jimatch, *w, *cheap, *js, *is, *ps, *Ai, *Cp, *jmatch, *imatch;
  cs *C;
  if (!A)
    return (NULL ); /* check inputs */
  n = A->n;
...
cs *cs_permute(const cs *A, const int *Pinv, const int *Q, int values) {
  ...
  for (k = 0; k < n; k++) {
    ...
    for (p = Ap[j]; p < Ap[j + 1]; p++) {
      if (Cx)
        Cx[nz] = Ax[p]; /* row i of A is row Pinv[i] of C */
...
csn *cs_qr(const cs *A, const css *S) {
  ...
  int i, k, p, m, n, vnz, p1, top, m2, len, col, rnz, *s, *leftmost, *Ap, *Ai, *parent, *Rp, *Ri, *Vp, *Vi, *w, *Pinv, *Q;
  cs *R, *V;
  csn *N;
  if (!A || !S || !S->parent || !S->Pinv)
    return (NULL ); /* check inputs */
  m = A->m;
  n = A->n;
  Ap = A->p;
...
int cs_scatter(const cs *A, int j, double beta, int *w, double *x, int mark, cs *C, int nz) {
  ...
  double *Ax;
  if (!A || !w || !C)
    return (-1); /* ensure inputs are valid */
  Ap = A->p;
  Ai = A->i;
  Ax = A->x;
  Ci = C->i;
...
int cs_sprealloc(cs *A, int nzmax) {
  ...
  if (A->nz >= 0)
    A->p = cs_realloc(A->p, nzmax, sizeof(int), &okj);
  if (A->x)
    A->x = cs_realloc(A->x, nzmax, sizeof(double), &okx);
  ok = (oki && okj && okx);
  if (ok)
    A->nzmax = nzmax;
  return (ok);
