...
namespace MBSimGUI {
  ...
  void MainWindow::selectSolver(int i) {
    ...
    if(element) {
      ...
      auto *ele = static_cast<DOMElement*>(doc->importNode(element,true));
...
//    shared_ptr<xercesc::DOMDocument> doc(static_cast<xercesc::DOMDocument*>(this->doc->cloneNode(true)));
  ...
  void MainWindow::saveElementAs() {
    ...
    QModelIndex index = elementView->selectionModel()->currentIndex();
    auto *element = static_cast<Element*>(model->getItem(index)->getItemData());
    QString file=QFileDialog::getSaveFileName(nullptr, "XML model files", QString("./")+element->getName()+".mbsim.xml", "XML files (*.xml)");
    if(not file.isEmpty()) {
