...
try:
  ...
  import platform
  import configparser
  # MISSING: remove this is mbsimenv/buildwin64 is using python >= 3.6
  def f(s, locals=None):
    import re
    formatRE=re.compile(r"{[^}]+}")
    ret=""
    last=0
    for m in re.finditer(formatRE, s):
      m.start(0)
      m.end(0)
      ret+=s[last:m.start(0)]+str(eval(m.group(0)[1:-1], None, locals))
      last=m.end(0)
    ret+=s[last:]
    return ret
  # MISSING END
  ...
  if platform.system()=="Linux":
    ...
    def createShortcut(shortcutFile, target, iconFile, workingDir=''):
      ...
      vb=f(r'''set shell=CreateObject("WScript.Shell")
  set sc=shell.CreateShortcut("{shortcutFile.replace("\\", "/")}")
  sc.TargetPath="{target.replace("\\", "/")}"
  sc.WorkingDirectory="{workingDir.replace("\\", "/")}"
  sc.IconLocation="{iconFile.replace("\\", "/")}"
  sc.Save
  ''', locals())
      ...
      for F in glob.glob(f("{FREEDESKTOPORGDIR}/mbsim-env.*.desktop")):
        ...
        if de["Type"]=="Application":
          ...
          if not addedAssociation:
            ...
            # desktop
            if copyToDesktop:
              createShortcut(f(r'{DESKTOP}/{de["Name"]}.lnk'), execFile, execFile+".exe,0")
  
            # start menu
            if copyToMenu:
              createShortcut(f(r'{STARTMENU}/{de["Name"]}.lnk'), execFile, execFile+".exe,0")
        if de["Type"]=="Link":
          url=de["URL"]
          ...
          # desktop
          if copyToDesktop:
            createShortcut(f(r'{DESKTOP}/{de["Name"]}.lnk'), url, de["IconWindows"])
