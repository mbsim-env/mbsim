...
namespace MBSimFlexibleBody {
  ...
  Vec FiniteElement1s21ANCF::StateBalken(const Vec& qElement, const Vec& qpElement, double s) {
    ...
    Vec X(6);
    X.set(RangeV(0,2), J*qElement);
    X.set(RangeV(3,5), J*qpElement);
    ...
    if(Euler) {      
      X.add(RangeV(3,4), v0*GlobalShapeFunctions_1stDerivative(s).T()*qElement);
  ...
  Mat FiniteElement1s21ANCF::JGeneralized(const Vec& qElement, double s) {
    ...
    J.set(RangeV(0,7),RangeV(0,1), GlobalShapeFunctions(s)(RangeV(0,7),RangeV(0,1)));
