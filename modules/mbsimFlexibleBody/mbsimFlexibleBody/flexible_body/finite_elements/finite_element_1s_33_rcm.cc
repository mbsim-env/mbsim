...
//    if(nrm2(qG-qG_Old)>tol_comp)
    ...
    JXqG.set(RangeV(0,15),RangeV(0,2), (drS+(tf->gettS()*depstil+(1.+tf->getepstil())*wt->gettSqI())*x
    ...
    Mat Z(3,16);
    Z.set(RangeV(0,0),RangeV(0,15), sin((tf->getpS())(1))*dwxdwt*wt->getw2coefqI()+x*dk0);
    Z(0,4) += cos((tf->getpS())(1))*w2(1);
    Z.set(RangeV(1,1),RangeV(0,15), dwxdwt*wt->getw1coefqI());
    Z.set(RangeV(2,2),RangeV(0,15), dwxdwt*wt->getw2coefqI());
    ...
    JXqG.set(RangeV(0,15),RangeV(3,3), (t(1)*ttqIt(RangeV(2,2),RangeV(0,15))+n(1)*ntqIt(RangeV(2,2),RangeV(0,15))+b(1)*btqIt(RangeV(2,2),RangeV(0,15))).T());
    JXqG.set(RangeV(0,15),RangeV(4,4), (t(2)*ttqIt(RangeV(0,0),RangeV(0,15))+n(2)*ntqIt(RangeV(0,0),RangeV(0,15))+b(2)*btqIt(RangeV(0,0),RangeV(0,15))).T());
    JXqG.set(RangeV(0,15),RangeV(5,5), (t(0)*ttqIt(RangeV(1,1),RangeV(0,15))+n(0)*ntqIt(RangeV(1,1),RangeV(0,15))+b(0)*btqIt(RangeV(1,1),RangeV(0,15))).T()); /* JR */
...
//      if(nrm2(qG-qG_Old)>tol_comp || nrm2(qGt-qGt_Old)>tol_comp)
  ...
  Vec FiniteElement1s33RCM::computeData(const Vec& qG, const Vec& qGt) {
    ...
    Vec Data(16);
    Data.set(RangeV(0,2), tf->getrS()); /* rS */
    Data.set(RangeV(3,5), qG(RangeV(13,15))-qG(RangeV(3,5))); /* dp */
    Data(6) = tf->getk0(); /* k0 */
    Data(7) = tf->getepstil(); /* epstil */
    Data.set(RangeV(8,10), tf->getrSt()); /* rSt */
    Data.set(RangeV(11,13), qGt(RangeV(13,15))-qGt(RangeV(3,5))); /* dpt */
