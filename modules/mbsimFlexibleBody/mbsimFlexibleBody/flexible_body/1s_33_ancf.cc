...
#include "mbsimFlexibleBody/flexible_body/fe/1s_33_ancf.h"
...
namespace MBSimFlexibleBody {
  ...
  void FlexibleBody1s33ANCF::GlobalMatrixContribution(int n, const fmatvec::Mat& locMat, fmatvec::Mat& gloMat) {
    ...
    if(n < Elements - 1 || openStructure==true) {
      gloMat.add(RangeV(j,j+11),RangeV(j,j+11), locMat);
    }
    else { // ring closure at finite element (end,1)
      gloMat.add(RangeV(j,j+5),RangeV(j,j+5), locMat(RangeV(0,5),RangeV(0,5)));
      gloMat.add(RangeV(j,j+5),RangeV(0,5), locMat(RangeV(0,5),RangeV(6,11)));
      gloMat.add(RangeV(0,5),RangeV(j,j+5), locMat(RangeV(6,11),RangeV(0,5)));
      gloMat.add(RangeV(0,5),RangeV(0,5), locMat(RangeV(6,11),RangeV(6,11)));
    }
  }
  ...
  void FlexibleBody1s33ANCF::updateJacobians(Frame1s *frame, int j) {
    ...
    if(currentElement<Elements-1 || openStructure) {
      Jacobian.set(RangeV(6*currentElement,6*currentElement+11),All, Jtmp);
    }
    else { // ringstructure
      Jacobian.set(RangeV(6*currentElement,6*currentElement+5),All, Jtmp(RangeV(0,5),All));
      Jacobian.set(RangeV(               0,                 5),All, Jtmp(RangeV(6,11),All));
    }
...
//    throwError("(FlexibleBody1s33ANCF::updateKinematicsForFrame): angularVelocity not implemented.");
  ...
  void FlexibleBody1s33ANCF::updateJacobians(NodeFrame *frame, int j) {
    ...
    Jacobian.set(RangeV(6*node+2,6*node+3),2, Jacobian(RangeV(6*node+2,6*node+3),2)/sqrt(q(4*node+2)*q(4*node+2)+q(4*node+3)*q(4*node+3)));
  ...
  void FlexibleBody1s33ANCF::init(InitStage stage, const InitConfigSet &config) {
    if(stage==unknownStage) {
      ...
      for(int i=0;i<Elements;i++) {
        qElement.emplace_back(12,INIT,0.);
        uElement.emplace_back(12,INIT,0.);
  ...
  void FlexibleBody1s33ANCF::BuildElements() {
    for(int i=0;i<Elements;i++) {
      ...
      if(i<Elements-1 || openStructure==true) {
        ...
        qElement[i].set(RangeV(0,5), q(RangeV(n,n+5)));
        uElement[i].set(RangeV(0,5), u(RangeV(n,n+5)));
        qElement[i].set(RangeV(6,11), q(RangeV(0,5)));
        uElement[i].set(RangeV(6,11), u(RangeV(0,5)));
      }
    }
    updEle = false;
  }
  ...
  void FlexibleBody1s33ANCF::initM() {
    for (auto & i : discretization)
      static_cast<FiniteElement1s33ANCF*>(i)->initM(); // compute attributes of finite element
    for (int i = 0; i < (int) discretization.size(); i++)
      GlobalMatrixContribution(i, discretization[i]->getM(), M); // assemble
    for (int i = 0; i < (int) discretization.size(); i++) {
      int j = 6 * i;
      LLM.set(RangeV(j, j + 5), facLL(M(RangeV(j, j + 5))));
      if (openStructure && i == (int) discretization.size() - 1)
        LLM.set(RangeV(j + 6, j + 11), facLL(M(RangeV(j + 6, j + 11))));
    }
  }
  ...
  void FlexibleBody1s33ANCF::initRelaxed(double alpha) {
    if(!initialised) {
      if(Elements==0)
        throwError("(FlexibleBody1s33ANCF::initRelaxed): Set number of finite elements!");
      ...
      if(openStructure) {
        ...
        for(int i=0;i<=Elements;i++) {
          q0Dummy.set(RangeV(6*i+0,6*i+1), direction*double(L/Elements*i));
