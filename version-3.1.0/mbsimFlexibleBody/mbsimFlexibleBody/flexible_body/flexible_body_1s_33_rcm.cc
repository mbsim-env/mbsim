...
#include<config.h>
#include "flexible_body_1s_33_rcm.h"

using namespace std;

namespace MBSimFlexibleBody {
  FlexibleBody1s33RCM::FlexibleBody1s33RCM(const std::string &name, bool openStructure_) :
  ...
  void FlexibleBody1s33RCM::GlobalVectorContribution(int n, const fmatvec::Vec& locVec, fmatvec::Vec& gloVec) {
  ...
  void FlexibleBody1s33RCM::GlobalMatrixContribution(int n, const fmatvec::Mat& locMat, fmatvec::Mat& gloMat) {
    int j = 10 * n; // start index in entire beam coordinates
  ...
  void FlexibleBody1s33RCM::GlobalMatrixContribution(int n, const fmatvec::SymMat16& locMat, fmatvec::SymMat& gloMat) {
    int j = 10 * n; // start index in entire beam coordinates
  ...
  void FlexibleBody1s33RCM::updateKinematicsForFrame(MBSim::ContourPointData &cp, MBSim::FrameFeature ff, MBSim::Frame *frame) {
  ...
  void FlexibleBody1s33RCM::updateJacobiansForFrame(MBSim::ContourPointData &cp, MBSim::Frame *frame) {
  ...
  void FlexibleBody1s33RCM::init(MBSim::InitStage stage) {
  ...
  void FlexibleBody1s33RCM::exportPositionVelocity(const std::string & filenamePos, const std::string & filenameVel /*= string( )*/, const int & deg /* = 3*/, const bool &writePsFile /*= false*/) {
#ifdef HAVE_NURBS
  ...
  void FlexibleBody1s33RCM::importPositionVelocity(const std::string & filenamePos, const std::string & filenameVel /* = string( )*/) {
#ifdef HAVE_NURBS
