...
namespace MBSimFlexibleBody {
  ...
  FlexibleBody1s33RCM::FlexibleBody1s33RCM(const string &name, bool openStructure_) :
      FlexibleBodyContinuum<Fixed<16>, double>(name), cylinder(new CylinderFlexible("Cylinder")), top(new FlexibleBand<Fixed<16> >("Top")), bottom(new FlexibleBand<Fixed<16> >("Bottom")), left(new FlexibleBand<Fixed<16> >("Left")), right(new FlexibleBand<Fixed<16> >("Right")), neutralFibre(new Contour1sFlexible<Fixed<16> >("NeutralFibre")), angle(new RevCardan()), Elements(0), L(0.), l0(0.), E(0.), G(0.), A(0.), I1(0.), I2(0.), I0(0.), rho(0.), R1(0.), R2(0.), epstD(0.), k0D(0.), epstL(0.), k0L(0.), openStructure(openStructure_), initialised(false), nGauss(3), cylinderRadius(0.), cuboidBreadth(0.), cuboidHeight(0.) {
  ...
  void FlexibleBody1s33RCM::BuildElements() {
    for (int i = 0; i < Elements; i++) {
      ...
      if (i < Elements - 1 || openStructure) {
        ...
        qElement[i].set(Index(0, 9), q(j, j + 9));
        uElement[i].set(Index(0, 9), u(j, j + 9));
        qElement[i].set(Index(10, 15), q(0, 5));
        if (q(j + 5) < q(5))
          qElement[i](15) -= 2. * M_PI;
        else
          qElement[i](15) += 2. * M_PI;
        uElement[i].set(Index(10, 15), u(0, 5));
      }
    }
  }
  ...
  void FlexibleBody1s33RCM::updateJacobiansForFrame(ContourPointData &cp, Frame *frame) {
    ...
    if (cp.getContourParameterType() == CONTINUUM) { // frame on continuum
      ...
      throw MBSimError("ERROR(FlexibleBody1s33RCM::updateJacobiansForFrame): ContourPointDataType should be 'NODE' or 'CONTINUUM'");
  ...
  void FlexibleBody1s33RCM::init(InitStage stage) {
    if (stage == unknownStage) {
      FlexibleBodyContinuum<Fixed<16>, double>::init(stage);
