...
namespace MBSimFlexibleBody {
  ...
  FlexibleBody1s21RCM::FlexibleBody1s21RCM(const string &name, bool openStructure_) :
      FlexibleBodyContinuum<Fixed<8>, double>(name), L(0), l0(0), E(0), A(0), I(0), rho(0), rc(0), dm(0), dl(0), openStructure(openStructure_), initialized(false) {
    contour1sFlexible = new Contour1sFlexible<Fixed<8> >("Contour1sFlexible");
  ...
  void FlexibleBody1s21RCM::BuildElements() {
    for (int i = 0; i < Elements; i++) {
      ...
      if (i < Elements - 1 || openStructure == true) {
        ...
        qElement[i].set(Index(0, 4), q(n, n + 4));
        uElement[i].set(Index(0, 4), u(n, n + 4));
        qElement[i].set(Index(5, 7), q(0, 2));
        if (qElement[i](2) - q(2) > 0.0)
          qElement[i](7) += 2 * M_PI;
        else
          qElement[i](7) -= 2 * M_PI;
        uElement[i].set(Index(5, 7), u(0, 2));
      }
    }
  }
  ...
  void FlexibleBody1s21RCM::GlobalMatrixContribution(int n, const fmatvec::SymMat8& locMat, fmatvec::SymMat& gloMat) {
    int j = 5 * n;
    ...
    if (n < Elements - 1 || openStructure == true) {
      ...
      gloMat(Index(j, j + 4)) += locMat(Range0x4());
      gloMat(Index(j, j + 4), Index(0, 2)) += locMat(Range0x4(), Range5x7());
      gloMat(Index(0, 2)) += locMat(Range5x7());
    }
  }
  ...
  void FlexibleBody1s21RCM::updateKinematicsForFrame(ContourPointData &cp, FrameFeature ff, Frame *frame) {
    if (cp.getContourParameterType() == CONTINUUM) { // frame on continuum
      ...
      if (ff == secondTangent || ff == cosy || ff == position_cosy || ff == velocity_cosy || ff == velocities_cosy || ff == all)
        cp.getFrameOfReference().getOrientation().set(2, -frameOfReference->getOrientation().col(2)); // binormal (cartesian system)
  ...
  void FlexibleBody1s21RCM::updateJacobiansForFrame(ContourPointData &cp, Frame *frame) {
    ...
    if (cp.getContourParameterType() == CONTINUUM) { // frame on continuum
      ...
      Mat8x3 Jtmp = static_cast<FiniteElement1s21RCM*>(discretization[currentElement])->JGeneralized(qElement[currentElement], sLocal);
  ...
  void FlexibleBody1s21RCM::init(InitStage stage) {
    if (stage == unknownStage) {
      FlexibleBodyContinuum<Fixed<8>, double>::init(stage);
  ...
  void FlexibleBody1s21RCM::plot(double t, double dt) {
    ...
    FlexibleBodyContinuum<Fixed<8>, double>::plot(t, dt);
  }
  ...
  void FlexibleBody1s21RCM::initInfo() {
    FlexibleBodyContinuum<Fixed<8>, double>::init(unknownStage);
    l0 = L / Elements;
    Vec g = Vec("[0.;0.;0.]");
    for (int i = 0; i < Elements; i++) {
      discretization.push_back(new FiniteElement1s21RCM(l0, A * rho, E * A, E * I, g));
