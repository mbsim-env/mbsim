...
namespace MBSimFlexibleBody {
  ...
  FlexibleBody1s23BTA::FlexibleBody1s23BTA(const string &name) :
      FlexibleBodyContinuum<Ref, double>(name), L(0), l0(0.), E(0), A(0), Iyy(0), Izz(0), rho(0), rc(0) {
    cylinderFlexible = new CylinderFlexible<Ref>("CylinderFlexible");
  ...
  void FlexibleBody1s23BTA::GlobalMatrixContribution(int n, const fmatvec::SqrMat& locMat, fmatvec::Mat& gloMat) {
   throw new MBSimError("ERROR: FlexibleBody1s23BTA::GlobalMatrixContribution: not implemented!");
  }
  ...
  void FlexibleBody1s23BTA::updateKinematicsForFrame(ContourPointData &cp, FrameFeature ff, Frame *frame) {
    ...
    if (cp.getContourParameterType() == CONTINUUM) { // frame on continuum
      ...
      if (ff == secondTangent || ff == cosy || ff == position_cosy || ff == velocity_cosy || ff == velocities_cosy || ff == all)
        cp.getFrameOfReference().getOrientation().set(2, -AWK.col(2)); // binormal (cartesian system)
      ...
      throw MBSimError("ERROR(FlexibleBody1s23BTA::updateKinematicsForFrame): ContourPointDataType should be 'NODE' or 'CONTINUUM'");
  ...
  void FlexibleBody1s23BTA::updateJacobiansForFrame(ContourPointData &cp, Frame *frame) {
    ...
    if (cp.getContourParameterType() == CONTINUUM) { // force on continuum
      ...
      throw MBSimError("ERROR(FlexibleBody1s23BTA::updateJacobiansForFrame): ContourPointDataType should be 'NODE' or 'CONTINUUM'");
  ...
  void FlexibleBody1s23BTA::init(InitStage stage) {
    if (stage == unknownStage) {
      ...
      cylinderFlexible->setAlphaStart(0);
      cylinderFlexible->setAlphaEnd(L);
      if (userContourNodes.size() == 0) {
        ...
        cylinderFlexible->setNodes(userContourNodes);
