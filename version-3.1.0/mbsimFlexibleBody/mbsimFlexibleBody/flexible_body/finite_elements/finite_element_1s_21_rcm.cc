...
namespace MBSimFlexibleBody {
  ...
  FiniteElement1s21RCM::FiniteElement1s21RCM(double sl0, double sArho, double sEA, double sEI, Vec sg) :
      l0(sl0), Arho(sArho), EA(sEA), EI(sEI), wss0(0.), depsilon(0.), g(sg), M(), h(), Dhq(), Dhqp(), Damp(), qLocal(), qpLocal(), hIntermediate(), MLocal(), Jeg(), Jegp(), qElement_Old(), qpElement_Old(), tol_comp(1e-8) {
    l0h2 = l0 * l0;
    l0h3 = l0h2 * l0;
    l0h4 = l0h3 * l0;
    l0h5 = l0h4 * l0;
  ...
  void FiniteElement1s21RCM::computeh(const Vec8& qElement, const Vec8& qpElement) {
    if (nrm2(qElement - qElement_Old) > tol_comp || nrm2(qpElement - qpElement_Old) > tol_comp) {
      ...
      double &aR = qLocal(6);
      double &bR = qLocal(7);
      double &phiSp = qpLocal(2);
      double &epsp = qpLocal(3);
      double &aLp = qpLocal(4);
      double &bLp = qpLocal(5);
      double &aRp = qpLocal(6);
      double &bRp = qpLocal(7);
  ...
  void FiniteElement1s21RCM::computedhdz(const Vec8& qElement, const Vec8& qpElement) {
    Mat Dhz(16, 8, INIT, 0.);
    Dhz = hFullJacobi(qElement, qpElement, qLocal, qpLocal, Jeg, Jegp, MLocal, hIntermediate);
    Dhq = Dhz(Index(0, 7), Index(0, 7));
    Dhqp = Dhz(Index(8, 15), Index(0, 7));
    Dhqp += Jeg.T() * Damp * Jeg;
  }

  double FiniteElement1s21RCM::computeKineticEnergy(const Vec8& qElement, const Vec8& qpElement) {
    Vec8 qLocal(NONINIT), qpLocal(NONINIT);
    ...
    double &xSp = qpLocal(0);
    double &ySp = qpLocal(1);
    double &phiSp = qpLocal(2);
    double &epsp = qpLocal(3);
    double &aLp = qpLocal(4);
    double &bLp = qpLocal(5);
  ...
  double FiniteElement1s21RCM::computeGravitationalEnergy(const Vec8& qElement) {
    Vec8 qLocal(NONINIT);
  ...
  double FiniteElement1s21RCM::computeElasticEnergy(const Vec8& qElement) {
    Vec8 qLocal(NONINIT);
  ...
  void FiniteElement1s21RCM::setCurlRadius(double R) {
    if (fabs(R) < epsroot())
      throw MBSimError("CurlRadius must not be 0!\n");
    wss0 = 1 / R;
  }
  ...
  Vec FiniteElement1s21RCM::LocateBeam(const Vec8& qElement, const double& s) {
    Vec8 qLocal;
    Vec8 qpLocalDummy;
  ...
  Vec FiniteElement1s21RCM::StateBeam(const Vec8& qElement, const Vec8& qpElement, const double& s) {
    Vec8 qLocal;
    Vec8 qpLocal;
    SqrMat8 Jeg;
    SqrMat8 Jegp;
  ...
  Mat8x3 FiniteElement1s21RCM::JGeneralizedInternal(const Vec8& qElement, const double& s) {
    Mat8x3 J;
  ...
  Mat8x3 FiniteElement1s21RCM::JGeneralized(const Vec8& qElement, const double& s) {
    SqrMat8 Jeg;
    BuildJacobi(qElement, Jeg);
    return Jeg.T() * JGeneralizedInternal(qElement, s);
  }

  Mat8x3 FiniteElement1s21RCM::JpGeneralized(const Vec8& qElement, const Vec8& qpElement, const double& s, const double& sp) {
    Mat8x3 Jp(NONINIT);

    Vec8 qLocal(NONINIT);
    Vec8 qpLocal(NONINIT);
    SqrMat8 Jeg;
    SqrMat8 Jegp;
  ...
  Vec8 FiniteElement1s21RCM::computeAdditionalElementData(Vec8 &qElement, Vec8 &qpElement) {
    Vec8 Data(NONINIT);

    Vec8 qLocal(NONINIT), qpLocal(NONINIT);
    SqrMat8 Jeg(NONINIT), Jegp(NONINIT);
  ...
  void FiniteElement1s21RCM::BuildqLocal(const Vec& qElement, Vec8& qLocal) {
    const double &x1 = qElement(0);
    const double &y1 = qElement(1);
    const double &phi1 = qElement(2);
    const double &a1 = qElement(3);
    const double &a2 = qElement(4);
    const double &x2 = qElement(5);
    const double &y2 = qElement(6);
  ...
  void FiniteElement1s21RCM::BuildJacobi(const Vec8& qElement, SqrMat8& Jeg) {
    const double &x1 = qElement(0);
    const double &y1 = qElement(1);
    const double &phi1 = qElement(2);
    const double &a1 = qElement(3);
  ...
  void FiniteElement1s21RCM::BuildJacobi(const Vec8& qElement, const Vec8& qpElement, SqrMat8& Jeg, SqrMat8& Jegp) {
    const double &x1 = qElement(0);
    const double &y1 = qElement(1);
    const double &phi1 = qElement(2);
    const double &a1 = qElement(3);
    ...
    const double &x1p = qpElement(0);
    const double &y1p = qpElement(1);
    const double &phi1p = qpElement(2);
    const double &a1p = qpElement(3);
    const double &a2p = qpElement(4);
    const double &x2p = qpElement(5);
    const double &y2p = qpElement(6);
  ...
  Vec6 FiniteElement1s21RCM::LocateLocalBeam(const Vec8& qLocal, const Vec8& qpLocal, const double& s, const bool calcAll) {
    Vec6 X; // x,y and phi | and | xp,yp and phip

    const double &xS = qLocal(0);
    const double &yS = qLocal(1);
    const double &phiS = qLocal(2);
    const double &eps = qLocal(3);
    const double &aL = qLocal(4);
    const double &bL = qLocal(5);
    const double &aR = qLocal(6);
    const double &bR = qLocal(7);

    const double &xSp = qpLocal(0);
    const double &ySp = qpLocal(1);
    const double &phiSp = qpLocal(2);
    const double &epsp = qpLocal(3);
    const double &aLp = qpLocal(4);
    const double &bLp = qpLocal(5);
    const double &aRp = qpLocal(6);
    const double &bRp = qpLocal(7);
  ...
  Mat16x8 FiniteElement1s21RCM::hFullJacobi(const Vec8& qElement, const Vec8& qpElement, const Vec8& qLocal, const Vec8& qpLocal, const SqrMat8& Jeg, const SqrMat8& Jegp, const SymMat8& MLocal, const Vec8& hIntermediate) {
    Mat16x8 Dhz;
    ...
    const double &hZ1 = hIntermediate(0);
    const double &hZ2 = hIntermediate(1);
    const double &hZ3 = hIntermediate(2);
    const double &hZ4 = hIntermediate(3);
    const double &hZ5 = hIntermediate(4);
    const double &hZ6 = hIntermediate(5);
    const double &hZ7 = hIntermediate(6);
    const double &hZ8 = hIntermediate(7);

    Vec JpqpG(8, fmatvec::INIT, 0.0);
    JpqpG = Jegp * qpElement;
    const double &JpqpG1 = JpqpG(0);
    const double &JpqpG2 = JpqpG(1);
    const double &JpqpG3 = JpqpG(2);
    const double &JpqpG4 = JpqpG(3);
    const double &JpqpG5 = JpqpG(4);
    const double &JpqpG6 = JpqpG(5);
    const double &JpqpG7 = JpqpG(6);
    const double &JpqpG8 = JpqpG(7);
    ...
    Dhz.set(Index(0, 7), Index(0, 7), dhqJ + Jeg.T() * (dhLq * Jeg - dhLqM * Jeg - MLocal * dhLqJp));
    Dhz.set(Index(8, 15), Index(0, 7), Jeg.T() * (dhLqp * Jeg - MLocal * dhLqpJp));
