...
//  specht->setRotation(new RotationAboutZAxis());
  ...
  specht->addContour(schnabel,SrSchabelspitze,SqrMat(3,EYE));//-SrSchwerpunkt
//  OpenMBV::Sphere* sph = new OpenMBV::Sphere;
//  sph->setRadius(r/50.);
//  schnabel->setOpenMBVRigidBody(sph); 

  RelativeRotationalSpringDamper *feder = new RelativeRotationalSpringDamper("Drehfeder");
//  muffe ->addFrame("COG",Vec(3));
//  specht->addFrame("COG",SrSchwerpunkt);//Vec(3));
