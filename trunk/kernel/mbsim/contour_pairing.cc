...
namespace MBSim {
  ...
  void ContourPairing::updateJacobians(double t) {
    //TODO: checkActiveg shouldn't be here!
    checkActiveg();
    if (isActive()) {
      contour1->updateJacobiansForFrame(cpData[0][0]);
      contour2->updateJacobiansForFrame(cpData[0][1]);
    }
  }
  ...
  void ContourPairing::init(InitStage stage) {
    if (stage == MBSim::preInit) {
      ...
      cpData[0][0].getFrameOfReference().getJacobianOfTranslation().resize();
      cpData[0][0].getFrameOfReference().getJacobianOfRotation().resize();
      cpData[0][1].getFrameOfReference().getJacobianOfTranslation().resize();
      cpData[0][1].getFrameOfReference().getJacobianOfRotation().resize();
