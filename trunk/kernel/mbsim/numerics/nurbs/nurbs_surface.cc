...
namespace MBSim {
  ...
  void NurbsSurface::globalInterp(const GeneralMatrix<fmatvec::Vec3 >& Q, const Vec& uk, const Vec& vk, int DegU, int DegV) {
    if (uk.size() != Q.rows())
      throw MBSimError("Error(NurbsCurve::globalInterpH: In the U direction, the length of lagrange parameters vectors is not equal to the number of the given interpolated points!)");
    if (vk.size() != Q.cols())
      throw MBSimError("Error(NurbsCurve::globalInterpH: In the V direction, the length of lagrange parameters vectors is not equal to the number of the given interpolated points!)");
...
//   cout << "fmatvec_suface: final control points for U and V direction" << P << endl;
  ...
  void NurbsSurface::globalInterpClosedU(const GeneralMatrix<fmatvec::Vec3 >& Q, const Vec& uk, const Vec& vk, int DegU, int DegV) {
    if (uk.size() != Q.rows() + DegU)
      throw MBSimError("Error(NurbsCurve::globalInterpH: In the U direction, the length of lagrange parameters vectors is not equal to the number of the given interpolated points plus degU!)");
    if (vk.size() != Q.cols())
      throw MBSimError("Error(NurbsCurve::globalInterpH: In the V direction, the length of lagrange parameters vectors is not equal to the number of the given interpolated points!)");
