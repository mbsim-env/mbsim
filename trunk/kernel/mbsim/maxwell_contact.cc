...
namespace MBSim {
      ...
      for (size_t potentialContact = 0; potentialContact < contourPairing.size(); potentialContact++) {
        ...
        if (pair->isActive()) { //if contact is active
          ...
          for (size_t i = 0; i < 2; i++) { //add forces to h-vector of both contours of the current contact point
            h[2 * potentialContact + i] += pair->getContourPointData()[i].getFrameOfReference().getJacobianOfTranslation().T() * pair->getForce(i);
            hLink[2 * potentialContact + i] += pair->getContourPointData()[i].getFrameOfReference().getJacobianOfTranslation().T() * pair->getForce(i);
          }
  ...
  void MaxwellContact::resizeJacobians(int j) {
    checkActiveg(); //TODO move it at another place
    for (vector<ContourPairing*>::iterator iter = contourPairing.begin(); iter != contourPairing.end(); iter++) {
      if ((*iter)->isActive()) {
        for (size_t i = 0; i < 2; i++)
          (*iter)->getContourPointData()[i].getFrameOfReference().resizeJacobians(j);
      }
    }
  }
