...
namespace MBSim {
  ...
  Actuator::Actuator(const string &name) : LinkMechanics(name), F(NULL), M(NULL) {}
  ...
  void Actuator::updateh(double t, int j) {
    ...
    if(F) {
      WF[1]=frame[0]->getOrientation()*forceDir * (*F)(t);
      WF[0] = -WF[1];
    }
    if(M) {
      WM[1]=frame[0]->getOrientation()*momentDir * (*M)(t);
      WM[0] = -WM[1];
    }
  ...
  void Actuator::plot(double t, double dt) {
    if(getPlotFeature(plotRecursive)==enabled) {
      if(getPlotFeature(generalizedLinkForce)==enabled) {
        if(F) {
          Vec f = (*F)(t);
  ...
  void Actuator::setForce(fmatvec::Mat dir, Function1<fmatvec::Vec,double> *func) {
    forceDir << dir;
  ...
  void Actuator::setMoment(fmatvec::Mat dir, Function1<fmatvec::Vec,double> *func) {
    momentDir << dir;
