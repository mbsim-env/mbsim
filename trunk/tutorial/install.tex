...
\end{verbatim}
as well as the procedures \textsc{automake, configure} and \textsc{install} in the directories \texttt{MBSim/HDF5Series} and \texttt{MBSim/HDF5Series/h5plotserie} . Last, \texttt{.bashrc} can be extended with
\begin{verbatim}
...
\end{verbatim} 
one should use the procedures \textsc{automake}, \textsc{configure} perhaps with FLAG \texttt{\-\-with-ocatve-path} and \textsc{install} in \texttt{MBSim/OpenMBV/mbxmlutils} for a static build of an independent XML preprocessor. The procedures \textsc{automake, configure} and \textsc{install} for a static build in \texttt{MBSim/OpenMBV} complete the installation. \texttt{make doc} produces a HTML documentation if invoked before the previous \textsc{install}.\\
