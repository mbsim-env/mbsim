...
\begin{verbatim}
 --with-allocator-class=--gnu_cxx::new_allocator
 --with-allocator-header=ext/new_allocator.h
\end{verbatim}
are necessary to use a standard thread-safe memory allocator instead of the \FMatVec built-in allocator. Details for adapting the installation can be found in \texttt{MBSim/fmatvec/README}. The code can be compiled and installed with the procedure \textsc{install}. For new installations one has to invoke the procedure \textsc{reinstall}.
...
\end{verbatim}
as well as the procedures \textsc{automake, configure} and \textsc{install} in the directories \texttt{MBSim/HDF5Serie/hdf5serie} and \texttt{MBSim/HDF5Serie/h5plotserie} . Last, \texttt{.bashrc} can be extended with
\begin{verbatim}
alias h5lsserie="MBSim/Install/h5lsserie"
alias h5dumpserie="MBSim/Install/bin/h5dumpserie"
alias h5plotserie="MBSim/Install/bin/h5plotserie"
\end{verbatim}
...
\end{verbatim} 
one should use the procedures \textsc{automake}, \textsc{configure} perhaps with FLAG \texttt{\-\-with-ocatve-path} and \textsc{install} in \texttt{MBSim/OpenMBV/mbxmlutils} for a static build of an independent XML preprocessor. The procedures \textsc{automake, configure} and \textsc{install} for a static build in \texttt{MBSim/OpenMBV/openmbv} complete the installation. \texttt{make doc} produces a HTML documentation if invoked before the previous \textsc{install}.\\
...
\item hdf5 with version 1.8.2 or newer (file handling),
\item HDF5Serie (wrapper for file handling).
\item openmbv/mbxmlutils (XML preprocessing).
\end{itemize}
...
\end{verbatim}
in \texttt{MBSim}. Then in single subdirectories the \texttt{kernel}, some \texttt{examples} and \texttt{modules} as well as the XML interface are extracted.\\
...
Now, one invokes the procedures \textsc{automake, configure} and \textsc{install} in the directories \texttt{MBSim/mbsim/kernel}, \texttt{MBSim/mbsim/modules/*} and \texttt{MBSim/mbsim/mbsimxml}. For a new installation the procedure \textsc{reinstall} is used. \texttt{make doc} creates a HTML documentation. 

%%------------------------------------------------------------ SUBSUBSECTION --
\subsection{MBSimAddOns}
{\small At the institute further modules can be found in \texttt{MBSimAddOn}: mechanical components~(\texttt{mbsimMechAdd}), hydraulic components~(\texttt{mbsimHydraulics}), and an interface for co-simulationen~(\texttt{mbsimCosim}). For installation one types
\begin{verbatim}
