...
The source code of \MBSim{} together with some examples and the necessary \FMatVec{} program can be found at \url{www.berlios.de} using subversion code administration\footnote{SVN Quick Reference Card: \url{http://www.cs.put.poznan.pl/csobaniec/edu/svn-refcard.pdf}}. Further, one needs \HDF{} at \url{http://www.hdfgroup.org} and a wrapper being only locally available from Markus Friedrich. \AMVis{} can be downloaded from \url{www.amm.mw.tum.de}. Everything is placed under \href{http://www.gnu.org/licenses/lgpl.html}{LGPL} (see file~\texttt{COPYING}, respectively).\\
...
{\small At the institute one can use the subversion checkout for the \HDF wrapper
\begin{verbatim}
 svn checkout http://zeiss/repos/SVN--rep/user/Friedrich/trunk/HDF5Serie
\end{verbatim}
 and use the procedures \textsc{automake, configure} and \textsc{install} in the directory \texttt{MBSim/HDF5Series}.
}
