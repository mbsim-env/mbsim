...
namespace MBSim {
  ...
  const int FiniteElement1s23BTA::nodalDOFs = 5;
  ...
  //------------------------------------------------------------------------------
  /*void FiniteElement1s23BTA::setMaterialDamping(double depsilons)//, const double& dws)
    {
    depsilon  = depsilons;
    Damp(3,3) = -depsilon;
    }
    void FiniteElement1s23BTA::setLehrDamping(double D)
    {
...
//    double &gy = g(1); double &gz = g(2);
  ...
  Vec FiniteElement1s23BTA::Tangent(Vec &q, double& s)
  {
    Vec tangent(3);
  ...
  {
    Vec X(12); // x,y,z und alpha und 2xBiegewinkel | und | zug. Geschwindigkeiten! TODO!!!!!
    ...
    //     double cosA = cos((a1*(l0 - s))/l0 + (a2*s)/l0);
    //     double sinA = sin((a1*(l0 - s))/l0 + (a2*s)/l0);
  ...
  //oooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooooo
  // JacobiMatrix der Lage des Balkenquerschnittes bei s 
  Mat FiniteElement1s23BTA::JGeneralized(Vec &q,const double& s) {
    // Rueckgagewert J: JacobiMatrix
    Mat J(2*nodalDOFs,6-1); // x-Richtung bleibt draussen
