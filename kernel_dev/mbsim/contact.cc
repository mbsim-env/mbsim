...
namespace MBSim {
  ...
  void Contact::initializeUsingXML(TiXmlElement *element) {
    ...
    GeneralizedForceLaw *gfl=ObjectFactory::createGeneralizedForceLaw(e->FirstChildElement());
    setContactForceLaw(gfl);
    gfl->initializeUsingXML(e->FirstChildElement());
    e=e->NextSiblingElement();
    GeneralizedImpactLaw *gifl=ObjectFactory::createGeneralizedImpactLaw(e->FirstChildElement());
    setContactImpactLaw(gifl);
    gifl->initializeUsingXML(e->FirstChildElement());
    e=e->NextSiblingElement();
    FrictionForceLaw *ffl=ObjectFactory::createFrictionForceLaw(e->FirstChildElement());
    setFrictionForceLaw(ffl);
    ffl->initializeUsingXML(e->FirstChildElement());
    e=e->NextSiblingElement();
    FrictionImpactLaw *fil=ObjectFactory::createFrictionImpactLaw(e->FirstChildElement());
