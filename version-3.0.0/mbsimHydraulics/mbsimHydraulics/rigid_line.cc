...
namespace MBSimHydraulics {
  ...
  void RigidLine::init(InitStage stage) {
    if (stage==MBSim::modelBuildup) {
      if (pL)
        static_cast<DynamicSystem*>(parent)->addLink(new RigidLinePressureLoss(name+"/LinePressureLoss", this, pL, false,false));
  ...
  void ClosableRigidLine::init(InitStage stage) {
    if (stage==MBSim::resolveXMLPath) {
      ...
      if (cpLBilateral)
        static_cast<DynamicSystem*>(parent)->addLink(new RigidLinePressureLoss(name+"/BilateralClosablePressureLoss", this, cpL, true, false));
      else
        static_cast<DynamicSystem*>(parent)->addLink(new RigidLinePressureLoss(name+"/ClosablePressureLoss", this, cpL, false,false));
  ...
  void UnidirectionalRigidLine::init(InitStage stage) {
    if (stage==MBSim::modelBuildup) {
      if (upL)
        static_cast<DynamicSystem*>(parent)->addLink(new RigidLinePressureLoss(name+"/RegularizedUnidirectionalPressureLoss", this, upL, false,false));
      else
        static_cast<DynamicSystem*>(parent)->addLink(new RigidLinePressureLoss(name+"/UnilateralUnidirectionalPressureLoss", this, NULL, false, true));
      RigidLine::init(stage);
    }
    else
      RigidLine::init(stage);
